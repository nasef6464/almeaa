...
import { mockSkills, mockQuestionBanks, mockSimTests, mockCourses, mockPackages, mockVideoLessons } from '../services/mockData';
import { Card } from '../components/ui/Card';
import { Video, BookOpen, FileText, PlayCircle, ChevronRight, Clock, CheckCircle, MonitorPlay, Star, User, ShoppingBag, ChevronDown, ChevronUp } from 'lucide-react';
import { ProgressBar } from '../components/ui/ProgressBar';
import { generateCourseSummary } from '../services/geminiService';
import { VideoModal } from '../components/VideoModal';

// Local Course Item with AI Summary (Copied/Shared Logic)
const CourseItem: React.FC<{ course: any }> = ({ course }) => {
    ...
    return (
        <Card className="flex flex-col h-full hover:shadow-xl transition-shadow duration-300 border border-gray-100 overflow-hidden group">
...
export const QudratSection: React.FC<QudratSectionProps> = ({ type: propType }) => {
    ...
    const isPackages = sectionType === 'packages';

    // Mock Data based on type
    const safeType = (sectionType === 'packages' ? 'quant' : sectionType) as keyof typeof mockSkills;
    
    const courses = mockCourses[safeType] || [];
    const skills = mockSkills[safeType] || [];
    const banks = mockQuestionBanks[safeType] || [];
    const tests = mockSimTests[safeType] || [];
        ...
        <div className="bg-gray-50 min-h-screen pb-20">
            ...
            <header className="bg-indigo-900 text-white py-12 relative overflow-hidden">
                <div className="max-w-7xl mx-auto px-4 relative z-10 text-center">
                    <h1 className="text-4xl font-bold mb-2">{titles[sectionType as keyof typeof titles]}</h1>
            ...
            <div className="max-w-7xl mx-auto px-4 py-8">
                ...
                <div className="flex flex-wrap justify-center gap-4 mb-12">
                    <TabButton active={activeTab === 'courses'} onClick={() => setActiveTab('courses')} icon={<MonitorPlay size={20} />} label="الدورات" />
                    <TabButton active={activeTab === 'skills'} onClick={() => setActiveTab('skills')} icon={<Video size={20} />} label="المهارات (فيديو)" />
                    <TabButton active={activeTab === 'banks'} onClick={() => setActiveTab('banks')} icon={<BookOpen size={20} />} label="أحدث البنوك" />
                    <TabButton active={activeTab === 'tests'} onClick={() => setActiveTab('tests')} icon={<FileText size={20} />} label="الاختبارات المحاكية" />
                </div>
                ...
                <div className="animate-fade-in">
                    ...
                    {activeTab === 'skills' && (
                        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {skills.map((skill) => {
                                ...
                                return (
                                    <Card key={skill.id} className={`p-0 border border-gray-100 transition-all ${isExpanded ? 'ring-2 ring-indigo-100' : 'hover:shadow-md'}`}>
                                        <div 
                    ...
                    {activeTab === 'banks' && (
                        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                            {banks.map((bank) => (
                                <Card key={bank.id} className="p-6 text-center border border-gray-100 hover:border-indigo-200 hover:shadow-lg transition-all">
                                    ...
                                    <Link 
                                        to={`/bank/${bank.id}`}
                                        className="block w-full bg-indigo-600 text-white py-2 rounded-lg font-bold text-sm hover:bg-indigo-700 transition-colors"
                                    >
                                        تصفح الأسئلة
                                    </Link>
                                </Card>
                            ...
                            {tests.map((test) => (
                                <div key={test.id} className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex flex-col md:flex-row items-center justify-between hover:border-indigo-300 transition-all">
                                    ...
                                    <Link 
                                        ...
                                        className="bg-white text-indigo-600 border-2 border-indigo-600 px-6 py-2 rounded-lg font-bold hover:bg-indigo-600 hover:text-white transition-colors flex items-center gap-2"
                                    >
                                        ابدأ الاختبار
