            ...
            <div className="max-w-6xl mx-auto px-4 -mt-8 mb-16 relative z-10">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                    {subjects.map(sub => (
                        <Link 
                            key={sub.id}
                            to={`/category/tahsili/${sub.id}`}
                            className="group relative h-24 md:h-36 w-full transition-all duration-300 focus:outline-none hover:-translate-y-1"
                        >
                            <div className={`absolute inset-0 bg-gradient-to-br ${sub.color} rounded-2xl shadow-lg transition-colors hover:shadow-xl`}></div>
                            <div className="absolute inset-0 flex flex-col items-center justify-center text-white p-2">
                                <span className="text-xl md:text-2xl font-black drop-shadow-md text-center">{sub.label}</span>
