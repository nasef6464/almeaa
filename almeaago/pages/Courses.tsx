...
import { Search, Filter, BookOpen, Clock, Star, ArrowRight, ChevronDown, ChevronUp, User, Book, List } from 'lucide-react';
                            ...
                            <div className="p-5 flex-1 flex flex-col bg-white">
                                ...
                                <div className="mb-4">
                                    <div className="flex justify-between text-xs text-gray-500 mb-1">
                                        ...
                                        <span>{course.syllabus?.length || 0} دروس</span>
                                    </div>
                                                ...
                                                {course.syllabus?.map((item, idx) => (
                                                    <div key={idx} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg text-xs">
                                                        <div className="flex items-center gap-2">
                                                            <div className={`w-2 h-2 rounded-full ${item.isCompleted ? 'bg-emerald-500' : 'bg-gray-300'}`}></div>
                                                            <span className={item.isCompleted ? 'text-gray-800 font-medium' : 'text-gray-600'}>{item.title}</span>
                                                        </div>
                                ...
                                <div className="mt-auto">
                                    <button 
                                        onClick={(e) => {
                                            e.stopPropagation(); // Prevent toggling expand when clicking action
                                            // Navigate logic would go here usually
                                        }}
                                        className="w-full bg-secondary-500 text-white py-2.5 rounded-xl font-bold text-sm hover:bg-secondary-600 transition-colors shadow-sm"
                                    >
